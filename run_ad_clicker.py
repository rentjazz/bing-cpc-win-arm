...
def start_tool(
    ...
    command.extend(["-q", query, "--id", str(browser_id)])
    if proxy:
    ...
    if result.returncode != 0:
        logger.warning(
            "ad_clicker.py exited with non-zero status "
            f"{result.returncode}. Check logs/adclicker.log for details."
        )
...
def main() -> None:
    ...
    if config.paths.proxy_file:
        ...
        proxies = [None]
        logger.info("No proxy_file configured. Running without proxies.")
